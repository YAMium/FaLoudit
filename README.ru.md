...
FaLoudit выполняет проверяемую техническую часть: определяет активный профиль MO2, индексирует поля локализации и сохранённые исходники скриптов, строит цепочку override-записей и связывает победивший plugin с модом в MO2. Codex или другой ИИ-инструмент с доступом к терминалу запускает команды, разбирает JSON-доказательства и возвращает понятный итог вместо массива сырых записей.
...
2. Скачайте `faloudit-codex-project-<версия>.zip`, например `faloudit-codex-project-0.4.0.zip`.
...
flowchart LR
    ...
    C -->|Да| D[Цепочка override-записи]
    C -->|Нет| E[Сохранённый исходник скрипта]
...
Индекс хранит поля локализации и сохранённый исходник скриптов, но не декомпилирует весь байткод. Статическое совпадение в скрипте доказывает наличие текста, но не выполнение кода во время игры. Строки внутри EXE, неподдерживаемый контент и скрипты без исходника могут потребовать ручного read-only поиска. FaLoudit сообщает об этих ограничениях, а не угадывает источник.
