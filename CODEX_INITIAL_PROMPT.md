...
Начиная с версии 0.4 до индексации обязательны явные `sourceLanguage` и
`targetLanguage`. Все упоминания English/Russian ниже являются главным
контрольным примером `en -> ru`, а production-модель и диагностика должны
использовать относительные роли source/target и выбранный профиль кодировки.
